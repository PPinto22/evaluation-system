...
public class SubmissionServiceImpl implements SubmissionService{
    ...
    private void addAnswersToSubmission(Submission submission, Map<Question, Answer> answers)
        ...
        for(QuestionSubmission qSub: submission._questionSubmissions.toArray()){
            if(qSub.isCorrect()){
                try {
                    QuestionScore qScore = examService.getQuestionScore(submission.get_exam(),qSub.get_question());
                    submission.setScore( submission.getScore() + qScore.getScore() );
                } catch (NonExistentEntityException e) {
